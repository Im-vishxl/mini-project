...
const AdminLoginPage = () => {
  ...
  try {
    const database_users = await axios.get(`${apiUrl}/api/adminUser`);
    ...
    // Compare user info
      if (userData) {
        if (userData.password !== password.value) {
          ...
          try {
            const generateToken = await axios.post(`${apiUrl}/api/adminLogin`, { user:submitted_user, password:submitted_pwd });
